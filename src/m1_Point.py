...
Authors: David Mutchler, Vibha Alangar, Dave Fisher, Amanda Stouder,
         their colleagues and Lara Peters.
...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance_traveled = self.get_distance_from(Point(x, y)) + self.distance_traveled
        self.x = x
        self.y = y
        self.number_of_moves = self.number_of_moves + 1

    def move_by(self, dx, dy):
        self.distance_traveled = self.distance_traveled + self.get_distance_from(Point(self.x + dx, self.y + dy))
        self.x = self.x + dx
        self.y = self.y +dy
        self.number_of_moves = self.number_of_moves +1
    ...
    def get_distance_from_start(self):
        p = Point(self.startx, self.starty)
